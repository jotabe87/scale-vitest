...
describe.skip('Dropdown Select', () => {
  ...
  it('should dispatch scale-change event when selecting the first option', () => {
    ...
    getInsideComboboxContainer().querySelector(`[part="combobox"]`).scrollIntoView = vi.fn()
    ...
    expect(spyChange.mock.lastCall[0].detail).toEqual(expect.objectContaining({ value: 'caspar' }))
  })
})